...
首页“使用前后有什么不同”现采用 TraeWork + Qwen 3.8 Max 的 NAC 真实测试；测试条件、未安装 Skill 的分享链接和安装后的完整证据文件见 [NAC 前后对比说明](nac-traework-before-after.md)。

`consumer-answer-demo.html` 现在是九个行为验收用例的统一画廊。每个案例都能展开结果说明图，并链接到详细答案、检索清单、筛选记录或方法资源。画廊与九张 PNG/SVG 由 `behavior-case-results.json` 确定性生成：
