...
<head>
  ...
  <meta name="description" content="evidence-based-nutrition-advisor V1.0.1 历史行为验收案例；当前产品为 V2.0">
  <title>历史案例（V1.0.1）｜evidence-based-nutrition-advisor</title>
  <style>
    ...
    *{{box-sizing:border-box}} body{{margin:0;color:var(--ink);background:var(--paper);font-family:"Microsoft YaHei","PingFang SC",system-ui,sans-serif;line-height:1.72}}
    ...
    .hero-note{{margin-top:26px;padding:16px 18px;border:1px solid #57779e;border-radius:14px;background:#ffffff0d;color:#e8f1fb}} .hero-note a{{color:#fff}}
...
<body>
  <header class="hero"><div class="hero-inner">
    <p class="kicker">EVIDENCE-BASED NUTRITION ADVISOR · V1.0.1 历史存档</p>
    <h1>早期行为验收案例</h1>
    <p class="lead">这里保存 V1.0.1 的十四个行为验收用例，供回归测试和追溯早期设计使用。部分卡片采用当时的 GRADE 流程，不能代表 V2.0 的 EAL 评价。</p>
    <div class="hero-note">当前版本示例请看 <a href="cases/glucosamine-2026-quick/answer.html">V2.0 氨糖软骨素证据卡</a>。本页旧案例仅用于查看历史产物，不作为今天的个人医疗建议。</div>
    <nav class="nav" aria-label="案例导航">{nav}</nav>
  </div></header>
  <main class="main">
    <section class="intro"><strong>怎么看：</strong>每张卡片都保留了当时的回答和检查记录。检索日期、评级方法、交互方式也按 V1.0.1 原样保存。要了解当前流程，请查看上方链接的 V2.0 示例。</section>
    {cases}
  </main>
  <footer>V1.0.1 历史案例 · 生成日期：{html.escape(payload['generated'])} · <a href="../tests/behavior-cases.md">查看验收规范</a> · <a href="../README.md">返回 V2.0 首页</a></footer>
</body>
</html>"""
