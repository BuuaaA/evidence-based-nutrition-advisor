...
def build_card(case: dict, generated: str) -> Canvas:
    ...
    canvas.text((MARGIN, 1287), "evidence-based-nutrition-advisor · 行为验收结果卡", 19, COLORS["muted"])
...
def build_contact_sheet(paths: list[Path]) -> Path:
    ...
    sheet = Image.new("RGB", (thumb_width * 3, thumb_height * 3), "#E8EEF6")
    for index, path in enumerate(paths):
        ...
        y = (index // 3) * thumb_height + (thumb_height - image.height) // 2
...
<head>
  ...
  <meta name="description" content="evidence-based-nutrition-advisor 的九个行为验收实跑案例">
  <title>普通用户可视化示例｜循证营养 Skill</title>
  <style>
...
<body>
  <header class="hero"><div class="hero-inner">
    <p class="kicker">EVIDENCE-BASED NUTRITION ADVISOR</p>
    <h1>普通用户可视化示例</h1>
    <p class="lead">九个行为验收用例的真实运行结果：既展示回答变得更可核查，也展示证据链不完整时系统如何拒绝给出假确定性。</p>
  ...
  <footer>生成日期：{html.escape(payload['generated'])} · <a href="../tests/behavior-cases.md">查看完整验收规范</a> · <a href="../README.md">返回项目首页</a></footer>
</body>
</html>"""
...
def main() -> None:
    ...
    if [case.get("id") for case in cases] != list(range(1, 10)):
        raise ValueError("behavior-case-results.json must contain case ids 1 through 9")
